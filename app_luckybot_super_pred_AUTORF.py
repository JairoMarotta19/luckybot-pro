...
if local_sp != visitante_sp:
    ...
    try:
        ...
        st.warning("⚠️ No se pudo cargar el modelo Random Forest. Entrenando uno nuevo...")
        df_rf = pd.read_csv("SP1.csv")
        equipos_rf = pd.concat([df_rf["HomeTeam"], df_rf["AwayTeam"]]).unique()
        equipo_map = {equipo: i for i, equipo in enumerate(equipos_rf)}
        df_rf["home_id"] = df_rf["HomeTeam"].map(equipo_map)
        df_rf["away_id"] = df_rf["AwayTeam"].map(equipo_map)
        df_rf["resultado"] = df_rf.apply(lambda row: "1" if row["FTHG"] > row["FTAG"] else "2" if row["FTHG"] < row["FTAG"] else "X", axis=1)
